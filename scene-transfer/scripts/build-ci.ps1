...
$work = Join-Path $env:RUNNER_TEMP 'scene-transfer-build'
...
target_sources(${CMAKE_PROJECT_NAME} PRIVATE src/plugin-main.cpp src/scene-transfer-dock.cpp src/scene-transfer-dock.hpp)
set_target_properties_plugin(${CMAKE_PROJECT_NAME} PROPERTIES OUTPUT_NAME ${_name})
'@ | Set-Content "$tpl\CMakeLists.txt" -Encoding UTF8

$buildspec = Get-Content "$tpl\buildspec.json" -Raw | ConvertFrom-Json
$buildspec.name = 'scene-transfer'
$buildspec.displayName = 'Scene Transfer'
...
$dll = Get-ChildItem "$tpl\release" -Recurse -Filter 'scene-transfer.dll' | Select-Object -First 1
if (-not $dll) { throw 'scene-transfer.dll was not produced.' }
$out = Join-Path $root 'dist'
$payload = Join-Path $out 'payload'
New-Item "$payload\obs-plugins\64bit" -ItemType Directory -Force | Out-Null
Copy-Item $dll.FullName "$payload\obs-plugins\64bit\scene-transfer.dll" -Force
