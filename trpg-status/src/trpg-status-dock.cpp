...
QString TRPGStatusDock::makeHtml() const {QString cards;for(const auto &c:chars){int pct=c.maxHp?c.hp*100/c.maxHp:0;QString state=pct>=60?"healthy":(pct>=30?"warning":"danger");cards+=QString("<div class='card %1'><div class='top'><b>%2</b><span>%3 / %4</span></div><div class='track'><div class='bar' style='width:%5%%'></div></div></div>").arg(state,c.name.toHtmlEscaped()).arg(c.hp).arg(c.maxHp).arg(pct);}return QString(R"HTML(<!doctype html><html><head><meta charset='utf-8'><style>html,body{margin:0;background:transparent;font-family:Arial,sans-serif;color:white}.wrap{padding:24px;width:720px;box-sizing:border-box}.card{margin:0 0 14px;padding:14px 16px;background:rgba(12,15,24,.88);border:2px solid rgba(255,255,255,.18);border-radius:14px}.top{display:flex;justify-content:space-between;font-size:24px;margin-bottom:9px}.track{height:24px;background:rgba(255,255,255,.12);border-radius:12px;overflow:hidden}.bar{height:100%%;transition:width .45s ease,background-color .25s ease}.healthy .bar{background:#39c86a}.warning .bar{background:#f29a2e}.danger .bar{background:#e94343;animation:pulse .8s infinite alternate}@keyframes pulse{to{filter:brightness(1.35)}}</style></head><body><div class='wrap'>%1</div></body></html>)HTML").arg(cards);}
