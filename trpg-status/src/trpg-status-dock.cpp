...
QString TRPGStatusDock::makeHtml() const {QString cards;for(const auto &c:chars){int pct=c.maxHp?c.hp*100/c.maxHp:0;QString state=c.hp<=0?"down":(pct<=30?"danger":"");cards+=QString("<div class='card %1'><div class='top'><b>%2</b><span>%3 / %4</span></div><div class='track'><div class='bar' style='width:%5%%'></div></div></div>").arg(state,c.name.toHtmlEscaped()).arg(c.hp).arg(c.maxHp).arg(pct);}return QString(R"HTML(<!doctype html><html><head><meta charset='utf-8'><style>html,body{margin:0;background:transparent;font-family:Arial,sans-serif;color:white}.wrap{padding:24px;width:720px;box-sizing:border-box}.card{margin:0 0 14px;padding:14px 16px;background:rgba(12,15,24,.88);border:2px solid rgba(255,255,255,.18);border-radius:14px}.top{display:flex;justify-content:space-between;font-size:24px;margin-bottom:9px}.track{height:24px;background:rgba(255,255,255,.12);border-radius:12px;overflow:hidden}.bar{height:100%%;background:#55d878;transition:width .45s ease}.danger .bar{background:#ff5a38;animation:pulse .8s infinite alternate}.down{filter:grayscale(1);opacity:.72}.down .bar{background:#777}@keyframes pulse{to{filter:brightness(1.45)}}</style></head><body><div class='wrap'>%1</div></body></html>)HTML").arg(cards);}
void TRPGStatusDock::updateOverlay(){QString dir=QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)+"/trpg-status";QDir().mkpath(dir);htmlPath=dir+"/hud.html";QFile f(htmlPath);if(f.open(QIODevice::WriteOnly|QIODevice::Truncate))f.write(makeHtml().toUtf8());obs_source_t *source=obs_get_source_by_name("TRPG Status HUD");obs_data_t *settings=obs_data_create();obs_data_set_bool(settings,"is_local_file",true);obs_data_set_string(settings,"local_file",htmlPath.toUtf8().constData());obs_data_set_int(settings,"width",768);obs_data_set_int(settings,"height",900);obs_data_set_int(settings,"fps",60);if(source){obs_source_update(source,settings);obs_source_media_restart(source);}else{source=obs_source_create("browser_source","TRPG Status HUD",settings,nullptr);if(source){obs_source_t *sceneSource=obs_frontend_get_current_scene();obs_scene_t *scene=sceneSource?obs_scene_from_source(sceneSource):nullptr;if(scene)obs_scene_add(scene,source);if(sceneSource)obs_source_release(sceneSource);}}obs_data_release(settings);if(source)obs_source_release(source);statusLabel->setText("HUD updated: TRPG Status HUD");}
